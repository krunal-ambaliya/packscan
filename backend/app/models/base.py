...
from sqlalchemy import String, DateTime, ForeignKey, Float, Text, Enum as SQLEnum, Sequence
from sqlalchemy.dialects.postgresql import UUID, JSONB
from sqlalchemy.orm import DeclarativeBase, Mapped, mapped_column, relationship

inspection_seq = Sequence('inspection_seq')
...
class Inspection(Base):
    ...
    overall_severity: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)
...
class ExtractedField(Base):
    ...
    bbox_json: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)  # [x, y, w, h]
    confidence: Mapped[float] = mapped_column(Float, default=0.90)
    font_mm: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    rule_min_mm: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
